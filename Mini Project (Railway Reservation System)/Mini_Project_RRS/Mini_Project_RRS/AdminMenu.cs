    ...
    public static class AdminMenu
    {
        public static void ShowMenu()
        {
            while (AuthService.LoggedInUserRole == "Admin")
            {
                ...
                Console.WriteLine("5. View All Reservations");
                ...
                switch (choice)
                {
                    ...
                    case "3": UpdateTrainDetails(); break;
                    case "4": SoftDeleteTrain(); break;
                    case "5": ViewAllReservations(); break;
                    case "6": ViewAllCancellations(); break;
                    case "7": AuthService.Logout(); Console.WriteLine("Logged out successfully."); Console.ReadKey(); break;
        ...
        public static void ViewAllTrains()
        {
            ...
            Console.WriteLine("---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
            Console.WriteLine($"{"Number",-10} {"Name",-20} {"Source",-15} {"Destination",-15} {"Duration",-10} {"Departure",-10} {"1AC Seats",-10} {"1AC Cost",-10} {"2AC Seats",-10} {"2AC Cost",-10} {"3AC Seats",-10} {"3AC Cost",-10} {"3E Seats",-10} {"3E Cost",-10} {"SL Seats",-10} {"SL Cost",-10}");
            Console.WriteLine("---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    try
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                Console.WriteLine($"{reader["TrainNumber"],-10} {reader["TrainName"],-20} {reader["Source"],-15} {reader["Destination"],-15} {reader["Duration"],-10} {((TimeSpan)reader["Departure"]).ToString("hh\\:mm"),-10} {reader["Seats_1AC"],-10} {reader["Cost_1AC"],-10} {reader["Seats_2AC"],-10} {reader["Cost_2AC"],-10} {reader["Seats_3AC"],-10} {reader["Cost_3AC"],-10} {reader["Seats_3E"],-10} {reader["Cost_3E"],-10} {reader["Seats_Sleeper"],-10} {reader["Cost_Sleeper"],-10}");
                            }
                        }
                    }
        ...
        public static void UpdateTrainDetails()
        {
            ...
            try
            {
                ...
                // You would present a menu here to select which field to update
                Console.Write("Enter the column name to update (e.g., TrainName, Source, Seats_1AC): ");
                ...
                using (SqlConnection connection = Connection.GetConnection())
                {
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        connection.Open();
                        int rows = command.ExecuteNonQuery();
                        Console.WriteLine(rows > 0 ? "Train details updated successfully!" : "Train not found or no changes made.");
                    }
                }
            }
            catch (Exception ex)
